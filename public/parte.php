...
function save_parte_record(PDO $pdo, array $enc, string $desde, string $hasta, string $titulo, string $fileRel): void {
  $stmt = $pdo->prepare("INSERT IGNORE INTO partes
...
<style>
  ...
  .table-responsive{ overflow:visible !important; } /* sin scroll en PDF */
...
$fileRel = 'files/partes/' . $fnBaseHuman . '.pdf';
...
// Rutas relativas para DB (PDF y HTML)
...
/* Guardado explícito (?save=1) — por si lo usás a futuro */
if ($wantSave) {
  if ($okPdf) {
    save_parte_record($pdo, $enc, $desde, $hasta, $tituloHumano, $fileRel);
...
if ($wantPdf) {
  if ($okPdf) {
    // También registramos si vino directo a ?pdf=1
    save_parte_record($pdo, $enc, $desde, $hasta, $tituloHumano, $fileRel);
