...
// Bases para assets y URLs
$BASE   = app_base();                    // p.ej. /ParteOficialTurno
$ASSETS = rtrim($BASE, '/') . '/public'; // p.ej. /ParteOficialTurno/public
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($email !== '') {
    ...
    // Para pruebas: mostrar enlace absoluto
    if ($token) {
      $scheme = is_https() ? 'https' : 'http';
      $link   = $scheme . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . url('reset.php') . '?token=' . urlencode($token);
    }
  }
  $sent = true;
}
...
<head>
  <meta charset="utf-8">
  <title>Recuperar contraseña – B Com 602</title>
        ...
        <div class="card-body">
          ...
          <?php if($sent): ?>
            <div class="alert alert-info">
              Si el email existe y está activo, se generó un enlace de recuperación.
            </div>
            <?php if($link): ?>
              <p class="mb-3"><strong>Enlace (pruebas):</strong> <a href="<?= h($link) ?>"><?= h($link) ?></a></p>
            <?php endif; ?>
          <?php endif; ?>
          ...
          <form method="post" action="<?= h($_SERVER['PHP_SELF']) ?>">
            ...
            <div class="d-flex align-items-center justify-content-between">
              ...
              <a href="<?= h(url('login.php')) ?>">Volver a iniciar sesión</a>
            </div>
          </form>
